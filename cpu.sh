...
sudo apt-get install -y --no-install-recommends gcc-12 g++-12 python3-dev ffmpeg
...
VLLM_TARGET_DEVICE=cpu uv pip install . --no-build-isolation # build backend returned an error - can be ignored for some reason
cd ..
