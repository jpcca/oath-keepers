...
class AgeDistribution(BaseModel):
    reasoning: str = Field(
        ...,
        description="Explanation of how the age distribution was derived based on the clues.",
        max_length=140,
    )
    ...
    def validate_distribution(self) -> "AgeDistribution":
        ...
        for i in range(len(sorted_bins) - 1):
            if sorted_bins[i].bin_end != sorted_bins[i + 1].bin_start:
                raise ValueError(
                    f"Gap or overlap between bins: [{sorted_bins[i].bin_start},{sorted_bins[i].bin_end}) and [{sorted_bins[i + 1].bin_start},{sorted_bins[i + 1].bin_end})"
                )
